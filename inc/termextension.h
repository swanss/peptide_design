...
public:
    /* Constructor */
    seedTERM();
    seedTERM(TermExtension* Fragmenter, vector<Residue*>_interactingRes, int _flankRes, bool search, mstreal max_rmsd, bool discard_match_data = false);
//    /*
...
//    seedTERM(const seedTERM& F);
    ...
    void searchForMatches(bool discard_match_data = false);
    ...
    bool operator<(seedTERM* other) {
        ...
        if (fragmentStructure.residueSize() == other->fragmentStructure.residueSize()) {
            // if not searched use the pointer to the central residue as an arbitrary yet consistent tie-breaker
            if ((!search) || (!other->search)) return cenRes < other->getCenRes();
    ...
    int getFlankingResPerInteractingRes() const {return flankRes;}
...
protected:
    ...
    /**
     Takes a single seedTERM and finds all possible expansions. Expansions can be generated by adding a residue that contacts the center one (which creates a new segment)
    ...
    vector<string> classifySegmentsUsingSTRIDELabels(int target_idx,vector<vector<int>> seed_segments);
