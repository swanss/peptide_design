...
int main(int argc, char *argv[]) {
    ...
    op.addOption("diversify", "If given, will find all matches (up to RMSD cutoff) and write out N randomly selected matches",false);
    ...
    if (!op.isGiven("diversify")) F.setMaxNumMatches(op.getInt("max_matches"));
    ...
    if (op.isGiven("diversify")) {
        vector<fasstSolution*> matches_reordered = matches.orderByDiscovery();
        int numToGet = min(op.getInt("diversify",50),matches.size());
        match_structures.resize(numToGet);
        for (int i = 0; i < numToGet; i++) {
            Structure& s = match_structures[i];
            F.getMatchStructure(*(matches_reordered[i]),s,detailed);
        }
    } else {
