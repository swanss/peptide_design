...
### `TERMExtensionBenchmark`

This program generates seeds and maps them to the peptide, reporting their RMSD if they fall within the given cutoff.
...
For comparison, null model seeds are also generated. Conceptually, these seeds match the overall density and length distribution of the TERM Extension seeds, but are 1) randomly sampled from the database and 2) have a random position/orientation with respect to the protein. Specifically there are two sets of null model seeds: type 1, which borrows the centroids from TERM Extension seeds, but have a randomized conformation/orientation and type 2, which are placed randomly within the bounding space and are otherwise the same as type 1.

The parameters to this program are as follows:

- `--pdb` *(required)*: a peptide-protein complex structure in the `.pdb` format.

- `--peptide` *(required)*: the chain ID of the peptide in the `.pdb` file, e.g. "B". Note: must be a single chain.
...
- `--max_rmsd` *(required)*: the maximum RMSD to consider when searching for seeds that map to the peptide. (recommended: 3)
...
- `--seq`: If selected, will constrain the matches by the sequence of the target at the central residue.
...
- `--ext_frag`: Skip all of the seed generation, and use this file instead.

The output is grouped into two directories:

`/termextension_output`

- A `.pdb` of each fragment

- seed binary files

  - `extendedfragments.info`: the name refers to the fact that each structure consists of a seed chain (always "0") and one or more chains that align to the protein.

  - `type1_seeds.bin`: each structure consists of the seed chain only

  - `type2_seeds.bin`: each structure consists of the seed chain only

- `*.lcloud` files for visualizing the seeds using SeedViewer

`/coverage_output`

- For each seed binary file there is

  - `*_aligned_seeds.tsv`: info on each aligned seeds

  - `*_covered_subsegments.tsv`: number of aligned seeds, per window, per RMSD bin.

  - `*_contacts.tsv`: all contacts between the peptide and protein

  - `*_covered_contacts.tsv`: number of aligned seeds, per contact, per RMSD bin.
